...
<div class="mb-20">
	<div class="blue-border-1 bg_f4fbfe_h pos-rel">
		...
		<div class="col61 dtc border-box p-20-0 pr-20">
			<? //print $data->performer_id .'<br>'. $this->app->user->id ?>
			<div class="profile-requests-btns pos-rel width100">
				<? if(!$this->app->user->isGuest && $data->performer_review == '' && $data->performer_id == $this->app->user->id)	{	?>
					<button class="show-full-review btn-blue-33 pos-abs">Оставить отзыв</button>
				<?	}	elseif(!$this->app->user->isGuest && $data->performer_review != '' && $data->performer_id == $this->app->user->id)	{	?>
	...
	<div class="requests-full-review blue-border-1 ov-hidden hide-block">
		...
		<? if($data->performer_review == '' && $data->performer_id == $this->app->user->id)	{	?>
			<form action="<?=$this->createUrl('/site/addnewreview')?>" method="post" class="clearfix p-20 bg_daf0fa">
				...
				<input type="hidden" name="fld" value="performer" />
				...
				<input type="submit" class="btn-send-review btn-blue-33 fRight" value="Разместить отзыв">
